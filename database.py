...
def get_top_songs(db_file_name):
    ...
    top_songs_dictionary_arry = sorted(
        new_dictionary_arry, key=itemgetter("shares"), reverse=True
    )
...
def add_song_to_database(data_base_name, artist, track, id):
    ...
    if is_found:
        ...
        write_db_w_insert(
            data_base_name, song_recommend_db, field_names, new_entry, entry_idx
        )
