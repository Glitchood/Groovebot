...
async def ping(interaction: discord.Interaction,
               hidden: discord.app_commands.Choice[int] = 0):
  displayname = interaction.user.display_name
  username = interaction.user.name
...
def top_embedaddfield(embed, rank, artist, track, popularity, shares, link):
  ...
  embed.add_field(name="",
                  value=f"**{rank}.⠀__{hyperlink}__    —    `{artist}`**",
                  inline=False)
  embed.add_field(
      name=f"",
      value=
...
async def embedFormat(interaction, artistName, trackName, albumName,releaseDate, popularity, duration, URL, image, shares,comment,followup):
...
async def share(interaction: discord.Interaction, artist_name: str, track_name: str, comment: str):
  username = interaction.user.name
  displayname = interaction.user.display_name
  ...
  if artist_name == '$':
    ...
    followup = True
    await embedFormat(interaction, query_artistName, query_trackName,query_albumName, query_releaseDate, query_popularity,query_duration, query_trackURL, query_albumImage,share_count,comment,followup)
